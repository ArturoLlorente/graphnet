...
from graphnet.models import StandardModelTito, Model, StandardModel
...
from graphnet.training.utils import make_dataloader, collate_fn
...
if __name__ == "__main__":
    ...
    test_dataloader = validation_dataloader ## TODO
    
    model.eval()
    model.inference()
    checkpoint_path = f'/remote/ceph/user/l/llorente/train_DynEdge_northeren_tracks/model_checkpoint_graphnet/DynEdgeTITO_northeren_tracks_direction_InIceDSTPulses_40e_p5_max_pulses500_layersize4_nb6-epoch=39-val_loss=0.029195.ckpt'
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader):
            ...
            preds.append(pred[0])
            event_no.append(batch.event_no)
            if validateMode:
    ...
    results['event_no'] = torch.cat(event_no).to('cpu').detach().numpy()
    if validateMode:
        results['zenith'] = torch.cat(zenith).to('cpu').numpy()
        results['azimuth'] = torch.cat(azimuth).to('cpu').numpy()
        
        
    results.to_csv(f"/remote/ceph/user/l/llorente/prediction_models/northeren_tracks/{run_name}_tito.csv")