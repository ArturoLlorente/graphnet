...
class ParquetDataset2(Dataset2):
    ...
    def reset_epoch(self) -> None:
        del self.this_batch, self.this_meta_arr
        gc.collect()
...
def collate_fn(graphs: List[Data]) -> Batch:
    ...
    for minp, maxp in zip([0]+NPLIST[:-1], NPLIST):
        ...
        #print(len(this_graphs), this_graphs[0].n_pulses.item(), this_graphs[-1].n_pulses.item())
        if len(this_graphs) > 0:
            this_batch = Batch.from_data_list(this_graphs, exclude_keys=exclude_keys)
            batch_list.append(this_batch)

    #TIMER[f'collate_fn'] += time.time()-st; st = time.time()
    #print(sorted(TIMER.items(), key=lambda x: x[1], reverse=True))
    return batch_list
...
class StandardModel2(Model):
    ...
    def training_step(self, train_batch: Data, batch_idx: int) -> Tensor:
        ...
        losses = self._tasks[0].compute_loss(preds, train_batch)
    ...
    def validation_step(self, val_batch: Data, batch_idx: int) -> Tensor:
        ...
        losses = self._tasks[0].compute_loss(preds, val_batch)
        loss = torch.sum(losses)
...
def build_model2(config: Dict[str,Any], train_dataloader: Any, train_dataset: Any) -> StandardModel2:
    ...
    detector = IceCubeKaggle2(
        graph_builder=KNNGraphBuilderMulti(nb_nearest_neighbours=NB_NEAREST_NEIGHBOURS, columns=COLUMNS_NEAREST_NEIGHBOURS)
    )
    gnn = DynEdge(
    ...
    model = StandardModel2(
        ...
        scheduler_kwargs={
            "milestones": [
                ...
                len(train_dataloader)*config["fit"]["max_epochs"]//(len(config['gpus'])*config['accumulate_grad_batches'][0]*(20/10)),
...
config = {
        "path": 'dummy',
        "inference_database_path": 'batch_660.db', #dummy
        ...
        'test_selection': None,
        'base_dir': 'training',
        'train_len': 0,                    #not using any more
        'valid_len': 0,                    #not using any more
        'train_max_pulse': 200,
        'valid_max_pulse': 200,
        'train_min_pulse': 0,
        'valid_min_pulse': 0,
...
# please set CKPT_FILE for restart
CKPT_FILE = None
if CKPT_FILE is not None:
    config["fit"]['ckpt_path'] = CKPT_FILE
...
callbacks = [
    ModelCheckpoint(
        dirpath='/remote/ceph/user/l/llorente/kaggle',
