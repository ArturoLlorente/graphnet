...
def batch_to_csr(batch):
    num_graphs = torch.unique(batch).size(0)
    num_nodes = batch.size(0)

    row_indices = torch.arange(num_nodes)
    csr_values = torch.ones(num_nodes)

    row_offsets = torch.zeros(num_graphs + 1)
    row_offsets.scatter_add_(0, batch, csr_values)

    return row_offsets, row_indices, csr_values
...
RESULTS_DIR = '/remote/ceph/user/l/llorente/graphnet/training'
...
for this_batch_id in batch_ids: #TODO
    ...
    #    print('skip', outfile)
    #    continue
    result0 = infer(0, 96, 1000, this_batch_id); gc.collect()
    result1 = infer(96, 140, 1000, this_batch_id); gc.collect()
