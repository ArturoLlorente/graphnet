    ...
    def forward(self, data: Data) -> Data:
        ...
        for idx in range(len(self._nb_nearest_neighbours)):
            ...
            if type(nb_nearest_neighbour) == str:
                (
                    nb_nearest_neighbour_min,
                    nb_nearest_neighbour_max,
                ) = nb_nearest_neighbour.split("-")
                nb_nearest_neighbour = torch.randint(
                    int(nb_nearest_neighbour_min),
                    int(nb_nearest_neighbour_max),
                    (1,),
                )[0]
